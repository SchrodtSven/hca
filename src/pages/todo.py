...
layout = html.Div(
    [
        ...
        html.Div(id="totals"),
        html.Details(
            children=[html.Summary("Click me Baby ..."), html.P("Lorem Ipsum ...")]
        ),
    ]
)
...
@callback(
    Output({"index": MATCH}, "style"), Input({"index": MATCH, "type": "done"}, "value")
)
def mark_done(done):
    return style_done if done else style_todo


@callback(Output("totals", "children"), Input({"index": ALL, "type": "done"}, "value"))
